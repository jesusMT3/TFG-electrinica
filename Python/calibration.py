...
def main():
    ...
    for i in plate:
        ...
        axs[0].plot(Xclean, coefficients[0] * Xclean +
                    coefficients[1], color='red', label='Regression line')
        axs[0].legend()
        ...
        # print coefficients
        print(f'{i}', ": V(G) = {:.4f} * G + {:.4f}. Error: {:.4f} %".format(
            round(coefficients[0], 4), round(coefficients[1], 4), round(coefficients[4], 4)))
